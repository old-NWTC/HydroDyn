MODULE HydroDyn
   ...
   !-------------------------------------------------------------------------------------------------
   TYPE(ProgDesc), PARAMETER, PUBLIC :: HD_Prog = ProgDesc( 'HydroDyn', '(v1.00.00b-jbj, 30-Sep-2011)' )   ! the name/version/date of the hydrodynamics program
...
!bjj end of proposed change v1.00.00c-bjj
   ...
   TYPE, PUBLIC :: HD_DataType                                                ! The type is public
      ...
      REAL(ReKi)                                :: LastCalcTime = 0.0         ! The last time an output was calculated
...
CONTAINS
!====================================================================================================
!SUBROUTINE HD_CalculateLoads( CurrentTime, X, XD, PtfmAM, PtfmFt, HD_Data, ErrStat )
...
! bjj: do we not care about HD_ConfigMarkers? Shouldn't that also be passed for future reference?
   ...
   REAL(ReKi),                 INTENT( IN    )     :: CurrentTime                ! Current simulation time (sec)    
...
!must return (1) discretization and (2) the dimension [0=point load, 1=load per unit length, 2=load per unit area, 3=load per unit volume]
   ...
   CALL InitWaves ( Waves_InitData, Current_Data, HD_Data%Waves_data, ErrStat   )      
...
!bjj rm v1.00.00a-bjj:         FltPtfm_InitData%X0       = HydroDyn_InitData%X0      
...
!      ! Local parameters
!      
!   INTEGER, PARAMETER                           :: MaxLineNodes = 200   ! Maximum number of LineNodes that can be read from a line
...
!   REAL(ReKi)                                   :: TmpAry(MaxLineNodes) ! temporary storage for line node locations read from each line
...
!rm   CALL ReadVar ( UnIn, FileName, Waves_InitData%WaveMod, 'WaveMod', 'Wave kinematics model switch', ErrStat )
...
!jbj: start of proposed change v1.00.00b-jbj
!rm   IF ( ( Waves_InitData%WaveMod /= 0 ) .AND. ( Waves_InitData%WaveMod /= 1 ) .AND. &
!rm        ( Waves_InitData%WaveMod /= 2 ) .AND. ( Waves_InitData%WaveMod /= 3 ) ) THEN
...
!jbj: start of proposed change v1.00.00b-jbj
!rm         CALL ProgAbort ( ' WaveMod must be 0, 1, 2, or 3.', .TRUE. )
...
!jbj: start of proposed change v1.00.00b-jbj
!rm         CALL ProgAbort ( ' WaveMod must be 0, 1, 2, 3, or 4.', .TRUE. )
...
!jbj: start of proposed change v1.00.00b-jbj
!rm   IF ( ( Waves_InitData%WaveMod == 1 ) .OR. ( Waves_InitData%WaveMod == 2 ) ) THEN   ! .TRUE if we have plane progressive (regular) or JONSWAP/Pierson-Moskowitz spectrum (irregular) waves, but not GH Bladed wave data.
!bjj: jmj implemented this:
!bjj:   IF ( ( WaveMod /= 0 ) .OR. ( WaveMod /= 3 ) .OR. ( WaveMod /= 4 ) )  THEN  ! .TRUE. for plane progressive (regular) or JONSWAP/Pierson-Moskowitz spectrum (irregular) wave
!bjj:but I think it should be this:
...
!jbj: start of proposed change v1.00.00b-jbj
!rm   IF ( ( Waves_InitData%WaveMod > 0 ) .AND. ( Waves_InitData%WaveMod /= 4 ) )  THEN   ! .TRUE if we have incident waves, but not GH Bladed wave data.
...
!         CALL ProgAbort ( ' RdtnDT must be greater than '//TRIM ( Flt2Lstr( RdtnTMax*EPSILON(RdtnDT) ) )//' seconds.', .TRUE. ) 
!         ErrStat = 1
!         CLOSE( UnIn )
!         RETURN   
!      END IF
...
!bjj is this correct????         ! also, jointzi(1) = 0 and jointzi(2)<0
         ...
         ELSE IF ( Set( Member(1)%SetIndx )%CA(1) /= Set( Member(1)%SetIndx )%CA(2)                            ) THEN      ! note comparison of floating points!
            CALL ExitThisRoutine( ' Morison member properties CA1 and CA2 must be zero for fixed bottom substructures.' )
            RETURN
         ELSE IF ( Set( Member(1)%SetIndx )%CD(1) /= Set( Member(1)%SetIndx )%CD(2)                            ) THEN      ! note comparison of floating points!
...
!====================================================================================================
!bjj start of proposed change v1.00.00c-bjj
!bjj FUNCTION HD_GetValue is renamed HD_GetValue_CHAR
!bjj FUNCTION HD_GetValue_AllOuts has been added
